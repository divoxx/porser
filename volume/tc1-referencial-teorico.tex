\begin{figure}
	\begin{center}
		...
		\caption{\label{fases} Estágios do processamento de linguagem natural}		
	\end{center}
\end{figure}
...
O processo de analise de sentença em linguagem natural é geralmente apresentado na literatura subdividido em vários níveis:
...
\begin{figure}
	\begin{center}
		...
		\caption{\label{tree} Árvore gramatical da frase \emph{O João vendeu para Maria o seu velho computador de mesa}}
...
A tabela \ref{tbl:corpora} lista alguns dos corpora anotado sintaticamente mais relevantes, que são da língua inglesa.
...
Existem outros corpus alem dos acima elencados, que possuem um numero menor de palavras. Três corpus da lista servem como marcos de referencia históricos:
...
\begin{table}
   ...
   \hline
    ...
    \hline
      ...
      \hline
		LS & Macador de item em listas      &		VB & Verbo infinitivo\\
      \hline
...
Para nosso estudos de desenvolvimento de um \emph{parser} probabilístico para a língua portuguesa e treino da ferramenta desenvolvida por Bikel , será utilizado o Bosque, parte da floresta sintática completamente revisada por lingüistas.
...
\begin{table}
   ...
   %%\setlength{\arrayrulewidth}{2\arrayrulewidth}
   %%\setlength{\belowcaptionskip}{10pt}
   \caption{\it Tags de Part-of-Speech da Floresta Sinática.}
...
O Corpus definido no projeto Semantic Share será detalhado posteriormente neste trabalho, em seção específica do projeto Semantic Share.
...
\begin{figure}
	\begin{center}
		...
		\caption{\label{schema} Estágios do processamento de linguagem natural}		
	\end{center}
\end{figure}

Uma visão geral do processo de \emph{parsing} estatístico pode ser observado na Figura \ref{schema}.
...
É semelhante ao de desenvolvimento, porem é proibida a análise qualitativa sobre o mesmo.

\end{enumerate}

O modulo de geração do \emph{parser} tem como entrada os exemplos do \emph{corpus} de treino e gera os parâmetros estatísticos que serão utilizados pelo \emph{parser} para tomar as decisões. Este módulo é parametrizável com informações linguísticas fornecidas pelo desenvolvedor, que guiam a interpretação do \emph{corpus} de treino. Por exemplo, a informação de que, quando um constituinte tem dois nomes seguidos, o núcleo é o da esquerda para o português e é o da direita para o inglês.
